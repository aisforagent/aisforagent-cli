...
describe('loadExtensions', () => {
  ...
  it('should load context file path when AIFA.md is present', () => {
    const workspaceExtensionsDir = path.join(
      tempWorkspaceDir,
      EXTENSIONS_DIRECTORY_NAME,
    );
    ...
    expect(ext1?.contextFiles).toEqual([
      path.join(workspaceExtensionsDir, 'ext1', 'AIFA.md'),
    ]);
...
function createExtension(
  ...
  if (addContextFile) {
    fs.writeFileSync(path.join(extDir, 'AIFA.md'), 'context');
  }
