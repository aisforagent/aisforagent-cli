...
describe('initCommand', () => {
  ...
  const geminiMdPath = path.join(targetDir, 'AIFA.md');
  ...
  it('should inform the user if AIFA.md already exists', async () => {
    ...
    expect(result).toEqual({
      ...
      messageType: 'info',
      content:
        'A AIFA.md file already exists in this directory. No changes were made.',
    });
    // Assert: Ensure no file was written
    expect(fs.writeFileSync).not.toHaveBeenCalled();
  });

  it('should create AIFA.md and submit a prompt if it does not exist', async () => {
